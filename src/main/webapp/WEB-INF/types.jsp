...
<form id="searchForm" method="get" action="${app}/goodstype/list">

    <input name="gtypeId" type="text" value="" placeholder="gtypeId"/>
    <input type="text" placeholder="gtypeName" name="gtypeName" value=""/>
...
<script>
    ...
    function gotoPage(pageNum, pageSize) {
        ...
        $.ajax({
            type: "GET",
            url: "${app}/goodstype/list?pageNum=" + pageNum + "&pageSize=" + pageSize,
            dataType: "json",
            data: $("#searchForm").serialize(),
            success: function (result) {
    ...
    function parseDataAndShow(result) {
        ...
        $.each(lists, function (index, item) {
            ...
            var uTr = $("<tr></tr>");
            //构建多个单元格
            var checkboxTh = $('<th><input type="checkbox" name="choiceList" value="${item.gtypId}"/></th>');
            var countTh = $('<th></th>').text(index + 1);
            var td1 = $('<td></td>').text(item.gtypId);
